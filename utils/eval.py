...
from .utils import post_process, box_iou
...
def Eval(model, test_loader):
    ...
    model.eval()
    with torch.no_grad():
        for inp, gt_boxes, cls_ids in tqdm(test_loader):
            ...
            for i, result in enumerate(results):
                ...
                cls_id = cls_ids[i][nozero_index]
                pred_boxes_list.append(pred_boxes)
                pred_labels_list.append(pred_labels)
                pred_scores_list.append(pred_scores)
                gt_boxs_list.append(gt_box)
                gt_labels_list.append(cls_id)
    # 计算precision与recall
    p, r, ap, f1, cls = calc_pr(pred_boxes_list, pred_labels_list, pred_scores_list, gt_boxs_list, gt_labels_list)
    return p, r, ap, f1, cls


def calc_pr(pred_boxes, pred_labels, pred_scores, gt_boxes, gt_labels):
    ...
    # 所有测试图片中预测的情况
    for pred_box, pred_label, pred_score, gt_box, gt_label in zip(pred_boxes, pred_labels, pred_scores, gt_boxes,
                                                                  gt_labels):
        ...
        for l in np.unique(gt_label):
            ...
            if len(pred_box_l) == 0:
                continue
            iou = box_iou(pred_box_l[:, None], gt_box_l[None, :])
            # 获取每个pred_box与多个gt_box_l最大iou的索引
            gt_index = iou.argmax(axis=1)
            # 那些iou小于iou_thresh的索引设为-1
            gt_index[iou.max(axis=1) < cfg.iou_threshold] = -1
